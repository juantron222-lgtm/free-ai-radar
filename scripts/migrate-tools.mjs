...
function main() {
  ...
  /*
   * Entries written directly in the v2 shape.
   *
   * Everything above is a translation of the legacy file. New editorial work
   * — model fichas in particular — has no legacy row to translate, and
   * inventing one just to run it through the converter would mean writing
   * v1 data we would then have to interpret. These are authored in the shape
   * the application actually consumes and merged here so that
   * `tools.json` stays generated by exactly one command.
   *
   * A v2 entry never silently replaces a migrated one: a slug collision is
   * reported and the legacy row wins, because a duplicate is a mistake and
   * the quiet resolution is the one that hides it.
   */
  const nativeRecords = existsSync(NATIVE)
    ? JSON.parse(readFileSync(NATIVE, 'utf8'))
    : [];
  ...
  for (const record of nativeRecords) {
    if (bySlug.has(record.slug)) {
      report.duplicates.push({
        slug: record.slug,
        kept: 'migrado desde tools.json',
        discarded: `${record.name} (tools-v2.json)`,
      });
      continue;
    }
    bySlug.set(record.slug, record);
  }
  report.nativeCount = nativeRecords.length;
