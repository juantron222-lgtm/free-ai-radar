...
import { checkReaderReady } from '../../../scripts/draft/legible.mjs';
 ...
 *
 * La implementación vive en `scripts/draft/legible.mjs`, por lo mismo que
 * `checkDraft`: la aplican este módulo y `scripts/newsroom-sync.mjs`, que corre
 * en `prebuild` y no puede importar TypeScript. Tenerla dos veces sería tenerla
 * distinta algún día, y la diferencia la descubriría un lector.
 *
 * La comprueban las dos rutas de la mesa —`canApprove`, y por tanto también
 * `canAutoPublish`— porque el fallo no era de la automática: era de que nadie
 * la comprobaba en ninguna de las dos.
 */
export { checkReaderReady };
